...
class SblamBase
{
	...
	{
		$sign = $isspam ? '>' : '<=';
		$q = "SELECT * from posts_meta left join posts_data on posts_meta.id = posts_data.id where spamscore $sign 0 and manualspam is null order by spamcert limit $start,$count";
		return $this->query($q);
	}
